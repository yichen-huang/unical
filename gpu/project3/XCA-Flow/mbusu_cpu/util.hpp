...
std::string converttostringint(int n)
{
	char result[20];
	sprintf(result, "%d", n);
	std::stringstream ss;
...
std::string converttostringdouble(double n)
{
	char result[20];
	sprintf(result, "%.1f", n);
	std::stringstream ss;
...
// ----------------------------------------------------------------------------
double* allocBuffer2D(int rows, int columns)
{
...
double* allocBuffer3D(int rows, int columns, int slices)
{
  double *tmp = (double*) malloc(sizeof(double) * rows * columns * slices);
  if (!tmp)
    return NULL;
  return tmp;
}

double* allocBuffer4D(int N, int rows, int columns, int slices)
{
  double *tmp = (double*) malloc(sizeof(double) * N * rows * columns * slices);
  if (!tmp)
    return NULL;
  return tmp;
}
