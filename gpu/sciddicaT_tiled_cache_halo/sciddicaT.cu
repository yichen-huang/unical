...
__global__ void cuda_sciddicaTFlowsComputation(int r, int c, double nodata, int* Xi, int* Xj, double *Sz, double *Sh, double *Sf, double p_r, double p_epsilon)
{
  ...
  int i_halo = row - MASK_WIDTH/2;
  int j_halo = col - MASK_WIDTH/2;

  __shared__ double Sz_tile[TILE_WIDTH+MASK_WIDTH-1][TILE_WIDTH+MASK_WIDTH-1];
  __shared__ double Sh_tile[TILE_WIDTH+MASK_WIDTH-1][TILE_WIDTH+MASK_WIDTH-1];
  ...
  if(i_halo >= 0 && i_halo < r && j_halo >= 0 && j_halo < c) {
    Sz_tile[threadIdx.y][threadIdx.x] = GET(Sz, c, i_halo, j_halo);
    Sh_tile[threadIdx.y][threadIdx.x] = GET(Sh, c, i_halo, j_halo);
  } else {
    Sz_tile[threadIdx.y][threadIdx.x] = 0.0f;
    Sh_tile[threadIdx.y][threadIdx.x] = 0.0f;
  }
  __syncthreads();

  if (row <= 0 || row >= r-1 || col <= 0 || col >= c-1) return;
  int i = threadIdx.y + MASK_WIDTH/2;
  int j = threadIdx.x + MASK_WIDTH/2;

  if (i <= 0 || i > TILE_WIDTH || j <= 0 || j > TILE_WIDTH) return;
  m = CUDA_GET(Sh_tile, i, j) - p_epsilon;
  u[0] = CUDA_GET(Sz_tile, i, j) + p_epsilon;

  for (int step = 1; step <= 4; step++) {
    z = CUDA_GET(Sz_tile, i+Xi[step], j+Xj[step]);
    h = CUDA_GET(Sh_tile, i+Xi[step], j+Xj[step]);
    u[step] = z + h;
  }
 ...
 __global__ void cuda_sciddicaTWidthUpdate(int r, int c, double nodata, int* Xi, int* Xj, double *Sz, double *Sh, double *Sf)
{
  ...
  __shared__ double Sf_tile[(TILE_WIDTH+MASK_WIDTH-1)*ADJACENT_CELLS][TILE_WIDTH+MASK_WIDTH-1];
  ...
  if(i_halo >= 0 && i_halo < r && j_halo >= 0 && j_halo < c)
    for(int step = 0; step < 4; step++)
      Sf_tile[threadIdx.y + step * (TILE_WIDTH+MASK_WIDTH-1)][threadIdx.x] = BUF_GET(Sf, r, c, step, i_halo, j_halo);
  else
    Sf_tile[threadIdx.y][threadIdx.x] = 0.0f;
  ...
  for (int step = 1; step <= 4; step++)
    h_next += CUDA_GET(Sf_tile, i+Xi[step]+(buf_count-step)*TILE_WIDTH, j+Xj[step]) - CUDA_GET(Sf_tile, i+(step-1)*(TILE_WIDTH+MASK_WIDTH-1), j);
